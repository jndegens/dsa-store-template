...
function StorePage({initialNiche}){
  ...
  useEffect(()=>{if(!active)return;const node=drawerRef.current;const getFocusable=()=>[...node.querySelectorAll('button,a,[tabindex]:not([tabindex="-1"])')].filter((el)=>!el.disabled);const firstControl=getFocusable()[0];firstControl?firstControl.focus():node?.focus();const key=(e)=>{if(e.key==='Escape')closePanel();if(e.key==='Tab'&&node){const focusable=getFocusable();if(!focusable.length)return;const first=focusable[0],last=focusable.at(-1);if(e.shiftKey&&(document.activeElement===first||document.activeElement===node)){e.preventDefault();last.focus()}else if(!e.shiftKey&&(document.activeElement===last||document.activeElement===node)){e.preventDefault();first.focus()}}};document.addEventListener('keydown',key);return()=>document.removeEventListener('keydown',key)},[activeId]);
  useEffect(()=>{if(!cart)return;const node=cartRef.current;const getFocusable=()=>[...node.querySelectorAll('button,a,[tabindex]:not([tabindex="-1"])')].filter((el)=>!el.disabled);const firstControl=getFocusable()[0];firstControl?firstControl.focus():node?.focus();const key=(e)=>{if(e.key==='Escape')closeCart();if(e.key==='Tab'&&node){const focusable=getFocusable();if(!focusable.length)return;const first=focusable[0],last=focusable.at(-1);if(e.shiftKey&&(document.activeElement===first||document.activeElement===node)){e.preventDefault();last.focus()}else if(!e.shiftKey&&(document.activeElement===last||document.activeElement===node)){e.preventDefault();first.focus()}}};document.addEventListener('keydown',key);return()=>document.removeEventListener('keydown',key)},[cart]);
  ...
  const copyPrompt=async()=>{try{if(!navigator.clipboard?.writeText)throw new Error('Clipboard niet beschikbaar');await navigator.clipboard.writeText(active.prompt);setCopyState('copied')}catch{setCopyState('error')}setTimeout(()=>setCopyState('idle'),1800)};
  const openCart=(value)=>{restoreRef.current=document.activeElement;setCart(value)};
  const closeCart=()=>{setCart(null);requestAnimationFrame(()=>restoreRef.current?.focus?.())};
  const add=()=>openCart({bundle,quantity,total:bundle.price*quantity});
  ...
  return <div className={`store ${store.fontClass} ${learning?'is-learning':''}`} style={theme}>
    ...
    <header className="header" id="top"><div className="brand-target learn-target" data-learn-id="brand"><a className="brand" href="#top"><i>{store.mark}</i><b>{store.brand}</b></a>{learning&&<Marker id="brand" label="Logo" onOpen={openPanel}/>}</div><nav>{store.navigation.map(n=><a key={n.href} href={n.href}>{n.label}</a>)}</nav><div className="header__actions"><button className={`learn-toggle ${learning?'on':''}`} onClick={toggleLearning} aria-pressed={learning}><Icon name="info"/>{learning?store.ui.learnOn:store.ui.learnOff}</button><button className="round" onClick={()=>openCart(cart||{bundle,quantity:0,total:0})} aria-label="Open demo winkelmand"><Icon name="cart"/><em>{cart?.quantity||0}</em></button><button className="round menu" onClick={()=>setMenu(!menu)} aria-label="Open menu"><Icon name={menu?'close':'menu'}/></button></div></header>
    ...
    <main id="main">
      <section className="pdp">
        ...
        <div className="buy"><div className="crumb">Home / {store.category}</div><span className="eyebrow">{store.product.eyebrow}</span><LearnTarget id="title" label="Titel" learning={learning} onOpen={openPanel} className="title"><h1>{store.product.name}</h1><p>{store.product.subtitle}</p></LearnTarget>
          <LearnTarget id="proof" label="Bewijs" learning={learning} onOpen={openPanel} className="rating-target"><a href="#reviews" className="rating"><Stars rating={store.product.rating}/><b>{store.product.rating}</b><span>{store.product.reviewCount} {store.ui.reviews}</span></a></LearnTarget>
    ...
    {active&&<><button className="overlay" onClick={closePanel} aria-label="Sluit uitleg"/><aside className="drawer" ref={drawerRef} tabIndex="-1" role="dialog" aria-modal="true" aria-labelledby="drawer-title"><header><div><span>LEERLAAG · {active.type}</span><h2 id="drawer-title">{active.label}</h2></div><button className="round" onClick={closePanel} aria-label="Sluit uitleg"><Icon name="close"/></button></header>{active.shot&&<div className="drawer__shot"><b>{active.shot.number}</b><span>{active.shot.shotType}</span><strong>1:1 · 1600 × 1600</strong></div>}<p className="drawer__guidance">{active.guidance}</p><div className="checklist"><b>Controleer dit</b>{active.checklist.map(x=><span key={x}><Icon name="check"/>{x}</span>)}</div><div className="prompt"><header><span>AI-PROMPT</span><button onClick={copyPrompt}><Icon name={copyState==='copied'?'check':'copy'}/>{copyState==='copied'?'Gekopieerd':copyState==='error'?'Kopiëren mislukt':'Kopieer'}</button><span className="sr-only" role="status" aria-live="polite">{copyState==='copied'?'Prompt gekopieerd.':copyState==='error'?'Prompt kon niet worden gekopieerd.':''}</span></header><code>{active.prompt}</code></div>{active.shot&&<dl><dt>Compositie</dt><dd>{active.shot.composition}</dd><dt>Achtergrond</dt><dd>{active.shot.background}</dd><dt>Belichting</dt><dd>{active.shot.lighting}</dd></dl>}<a href={`/stores/${store.slug}/template.json`}>Bekijk volledige JSON <Icon name="arrow"/></a></aside></>}
    {cart&&<><button className="overlay" onClick={closeCart} aria-label="Sluit winkelmand"/><aside className="cart" ref={cartRef} tabIndex="-1" role="dialog" aria-modal="true" aria-labelledby="cart-title"><header><span>DEMO-WINKELMAND</span><button className="round" onClick={closeCart} aria-label="Sluit winkelmand"><Icon name="close"/></button></header><h2 id="cart-title">{cart.quantity?'Goede keuze.':'Je winkelmand is leeg.'}</h2>{cart.quantity>0&&<div className="cart__item"><MediaCard media={store.product.media[0]} compact/><span><b>{cart.bundle.label}</b><small>{store.product.name} · {cart.quantity}×</small></span><b>{money(cart.total)}</b></div>}<p>{store.ui.demo}</p><button className="cta" onClick={closeCart}>{cart.quantity?'Rond demo af':'Verder winkelen'} <Icon name="arrow"/></button></aside></>}
  </div>
}
